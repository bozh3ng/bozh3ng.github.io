...
Calculation: $\mathbf{u} \times \mathbf{v}=\lVert \mathbf{u}\rVert\lVert \mathbf{v}\rVert \sin (\theta) \mathbf{n}$ where $\mathbf{n}$ is a unit vector perpendicular to the plane containing $\mathbf{u}$ and $\mathbf{v}$,
...
# Category
## Category product
[Category Product]({% post_url 2025-11-01-category-product %})
## Tensor Product
