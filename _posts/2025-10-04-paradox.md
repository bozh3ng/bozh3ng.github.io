...
- If $c=i$, the host randomly picks one of the two remaining goats, so $h\in\lbrace1,2,3\rbrace \backslash\lbrace c\rbrace$ with probability $\frac{1}{2}$ each.  
- If $c \neq i$, there is only one goat door available, the host must open it, $h\in\lbrace1,2,3\rbrace \backslash\lbrace c,i\rbrace$ with probability 1
...
Random host : Without looking, the host chooses uniformly at random one of the two doors $\lbrace1,2,3\rbrace \backslash\lbrace I\rbrace$. Call that door $H$. The door $H$ happens to be a goat. 
