...
- If $c=i$, the host randomly picks one of the two remaining goats, so $h\in\lbrace1,2,3\rbrace \backslash\lbracec\rbrace$ with probability $\frac{1}{2}$ each.  
- If $c \neq i$, there is only one goat door available, the host must open it, $h\in\lbrace1,2,3\rbrace \backslash\lbracec,i\rbrace$ with probability 1
...
$$E=\lbraceI=1, H=3\rbrace \in \mathcal{F}$$

Whether we win or not depends on $C$. Define events under condition $E$

- Stay-win: $T=\lbraceC=I\mid E\rbrace$
- Switch-win: $S=\lbraceC \neq I\mid E\rbrace$
...
## Case 2: Random Host
Same as before 
$C \in\lbrace1,2,3\rbrace$, $P(C=c)=1 / 3$.
$I \in\lbrace1,2,3\rbrace$, $P(I=i)=1 / 3$.
Random host : Without looking, the host chooses uniformly at random one of the two doors $\lbrace1,2,3\rbrace \backslash\lbraceI\rbrace$. Call that door $H$. The door $H$ happens to be a goat. 
...
$$  
A=\lbrace\lbrace1\rbrace,\lbrace2,3\rbrace,\lbrace4,5,6\rbrace\rbrace, \quad x=\lbrace1\rbrace \in A  
$$  

- Elements of $x$ : 1 .  
- Elements of $A$ : $\lbrace1\rbrace,\lbrace2,3\rbrace,\lbrace4,5,6\rbrace$.  
...
Each element $n \in \omega$ is the set $\lbrace0,1, \ldots, n-1\rbrace$. $\operatorname{rk}(n)=n$ is finite.  
...
Consequence: $x \notin x$ for all sets $x$. (Apply the axiom to $A=\lbracex\rbrace$, then we have $x \cap\lbracex\rbrace=\varnothing$, so $x \notin x$.)
