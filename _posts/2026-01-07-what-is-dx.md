...
The general idea: $dx$ is a 1-form that extracts the $x$-component of a tangent vector. $dx(v)$ gives the components of $v$ in the $x$-coordinate system. In $\mathbb{R}^n$ with standard coordinates, $dx_i$ is literally the projection onto the $i$-th coordinate where $i\in \lbracen\rbrace$.
...
The basis 1-forms $\lbracedx_i\rbrace$ are dual to the basis tangent vectors $\lbrace\partial/\partial x_j\rbrace$: $dx_i(\partial/\partial x_j) = \delta_i^j$. This duality is the same one that appears in the pushforward/pullback correspondence.
