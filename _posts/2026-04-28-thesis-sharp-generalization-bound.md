...
As argued in [Part 2 - Group Structure](/blog/2026/thesis-group-structure/), valid gradient descent should cross the orbit, because within an orbit all weights are functionally equivalent, this is the reparametrization symmetry of neural networks. In standard training this is trivial: since the objective is to minimize loss, and the loss is constant along orbits, the gradient is automatically orthogonal to orbit directions. No special machinery is needed.
...
$$
S_{\text{full}}^\epsilon(\theta)=\max_{\delta \in \mathbb{R}^d,\|\delta\| \leq \epsilon}[L(\theta+\delta)-L(\theta)]
$$

we define quotient sharpness:

$$
S_{\text{quot}}^\epsilon(\theta)=\max_{\delta \in \mathcal{H}_\theta,\|\delta\| \leq \epsilon}[L(\theta+\delta)-L(\theta)]
$$
