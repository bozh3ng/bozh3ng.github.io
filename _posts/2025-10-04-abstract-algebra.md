...
We denote the identity in $\langle{R}, \cdot\rangle$ as $1$, which is also called **unity**. The identity in $\langle R,+\rangle$ as $0$. Note that $0 \neq 1$; if $0 = 1$, then for any $a \in R$, we would have $a = a \cdot 1 = a \cdot 0 = 0$, so the ring collapses to the trivial zero ring $\lbrace0\rbrace$. If $\langle\mathrm{R}, \cdot\rangle$ has inverse element for some $u \in {R}, u$ is called **unit**.  
...
$$(a, b)=\lbracex a+y b \mid x, y \in \mathbb{Z}\rbrace$$
